...
function getBaseUrl() {
  ...
  if (isLocalHost) {
    return ENV_API_BASE_URL || LOCAL_API_BASE_URL;
  }

  if (ENV_API_BASE_URL && !envPointsToLocalhost) {
...
export const api = {
  ...
  exportAdminRegistrations: async (token, params = {}) => {
    const baseUrl = getBaseUrl();
    const response = await fetch(`${baseUrl}/admin/registrations/export${createAdminQuery(params)}`, {
      headers: createAuthHeaders(token),
    });
